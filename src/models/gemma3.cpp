...
void llama_model_gemma3::load_arch_tensors(llama_model_loader &) {
    ...
    for (int i = 0; i < n_layer; ++i) {
        ...
        // BitNet per-projection norms (optional, for I2_S models)
        ...
        // BitNet per-projection norms for FFN (optional)
...
llama_model_gemma3::graph<iswa>::graph(const llama_model & model, const llm_graph_params & params) : llm_graph_context(params) {
    ...
    for (int il = 0; il < n_layer; ++il) {
        ...
        // self-attention
        {
            // BitNet per-projection norms before Q/K/V
            ...
            ggml_tensor * Qcur = build_lora_mm(model.layers[il].wq, q_inp, model.layers[il].wq_s);
            cb(Qcur, "Qcur", il);
            ggml_tensor * Kcur = build_lora_mm(model.layers[il].wk, k_inp, model.layers[il].wk_s);
            cb(Kcur, "Kcur", il);
            ggml_tensor * Vcur = build_lora_mm(model.layers[il].wv, v_inp, model.layers[il].wv_s);
            cb(Vcur, "Vcur", il);
            ...
            if (model.layers[il].attn_out_norm_in) {
                // BitNet per-projection norm for o_proj: attention without wo, then norm + wo
        ...
        {
            // BitNet per-projection norms for FFN
            ggml_tensor * gate_inp = cur;
            ggml_tensor * up_inp = cur;

            if (model.layers[il].ffn_gate_norm_in) {
                gate_inp = build_norm(cur, model.layers[il].ffn_gate_norm_in, NULL, LLM_NORM_RMS, il);
                cb(gate_inp, "ffn_gate_norm_in", il);
            }
            if (model.layers[il].ffn_up_norm_in) {
                up_inp = build_norm(cur, model.layers[il].ffn_up_norm_in, NULL, LLM_NORM_RMS, il);
                cb(up_inp, "ffn_up_norm_in", il);
            }

            // gate and up projections (possibly with different inputs if norm_in exists)
            ggml_tensor * ffn_gate_out = build_lora_mm(model.layers[il].ffn_gate, gate_inp);
            cb(ffn_gate_out, "ffn_gate", il);
            ggml_tensor * ffn_up_out = build_lora_mm(model.layers[il].ffn_up, up_inp);
            cb(ffn_up_out, "ffn_up", il);

            // GELU activation on gate
            ffn_gate_out = ggml_gelu(ctx0, ffn_gate_out);
            cb(ffn_gate_out, "ffn_gate_gelu", il);

            // element-wise multiply
            cur = ggml_mul(ctx0, ffn_gate_out, ffn_up_out);
            cb(cur, "ffn_gate_up", il);

            // down projection with optional norm_in
            if (model.layers[il].ffn_down_norm_in) {
                cur = build_norm(cur, model.layers[il].ffn_down_norm_in, NULL, LLM_NORM_RMS, il);

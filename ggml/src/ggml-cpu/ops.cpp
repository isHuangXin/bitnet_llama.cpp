...
// ggml_compute_forward_mul_id

static void ggml_compute_forward_mul_id_f32(
        const ggml_compute_params * params,
        ggml_tensor * dst) {
    ...
    for (int ir = ir0; ir < ir1; ++ir) {
        ...
        // src1 indices: select expert by ids
        const int i11 = *(int32_t *) ((char *) src2->data + i1*nb20 + i2*nb21);
...
void ggml_compute_forward_mul_id(
        const ggml_compute_params * params,
        ggml_tensor * dst) {
    ...
    switch (src0->type) {
        case GGML_TYPE_F32:
            {
                ggml_compute_forward_mul_id_f32(params, dst);
            } break;
        default:
            {
                GGML_ABORT("unsupported type for ggml_compute_forward_mul_id: %s", ggml_type_name(src0->type));
            }
    }
}
